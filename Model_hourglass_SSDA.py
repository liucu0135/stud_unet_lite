...
class Discriminator(nn.Module):
    def __init__(self, out_ch=3, bn=False,para_reduce=1):
        ...
        layers = [Rse_block((256+64)//para_reduce, 64//para_reduce, bn=bn, single=True),
                  Rse_block(64//para_reduce, 64//para_reduce, bn=bn, single=True),
...
class Classifier(nn.Module):
    def __init__(self, out_ch=3, bn=False, para_reduce=1):
        ...
        layers = [torch.nn.MaxPool2d(3,2,1),
                  torch.nn.Conv2d(64//para_reduce,32,3,1,1),
                  torch.nn.ReLU(),
                  ...
                  torch.nn.ReLU(),]
        # layers = [Rse_block(256//para_reduce, 64, bn=bn, single=True),
        #           Rse_block(64, 64, bn=bn, single=True),
        #           Rse_block(64, 8, bn=bn, single=True)]
        # self.E2 = Rse_block(32, 32, bn=bn)
        # self.E3 = Rse_block(64+32, 64, bn=bn)
...
class Regressor(nn.Module):
    def __init__(self, out_ch=3, bn=True, para_reduce=1):
        super(Regressor, self).__init__()
        self.D3 = Rse_blockT(512//para_reduce + 256//para_reduce, 256//para_reduce, bn=bn)
        self.D2 = Rse_blockT(256//para_reduce + 128//para_reduce, 128//para_reduce, bn=bn)
        self.D1 = Rse_blockT(128//para_reduce + 64//para_reduce, 64//para_reduce, bn=bn, last=True)
        ...
        self.short3 = Short_cut_block(256//para_reduce, 256//para_reduce, bn=bn)
...
# class Regressor(nn.Module):
#     def __init__(self, out_ch=3, bn=True, single=False):
#         super(Regressor, self).__init__()
#         # layers = [Rse_block(512+256, 128, bn=True, single=True),
#         #           Rse_block(128, 64, bn=True, single=True),
#         #           Rse_block(64, 8, bn=True, single=True)]
#         layers = [Rse_block(256, 128, bn=bn, single=single),
#                   # nn.Dropout2d(0.5),
#                   Rse_block(128, 64, bn=bn, single=single),
#                   # nn.Dropout2d(0.5),
#                   Rse_block(64, 8, bn=bn, single=single)]
...
class SUNET(nn.Module):
    def __init__(self, in_ch=3, out_ch=2, ss=True, num_puzzle=9, scale_lr=1, multitask=False, para_reduce=1):
    ...
    def forward(self, data, ss=True, g=True):
        ...
        if ss:
            ...
            if g:
                ...
                e1_ri, e2_ri, e3_ri, e4_ri = self.extractor(self.input_ri)

                self.d_pre = torch.cat((self.disc(e1_nm, e2_nm, e3_nm, e4_nm), self.disc(e1_ri, e2_ri, e3_ri, e4_ri)),
                                       dim=0)
    ...
    def accuracy_class(self):
        ...
        if self.c_pre.shape[0] == self.c_label.shape[0]:
    ...
    def update_g(self, ss_only=False):
        self.cal_loss_g(ss_only)
        if not ss_only:
            l = self.Loss_rec_ri + self.Loss_rec_nm + self.Loss_g/100
        else:
            l = self.Loss_rec_ri / 2 + self.Loss_rec_nm / 2
