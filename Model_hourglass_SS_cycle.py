...
class Regressor(nn.Module):
    def __init__(self, out_ch=3, bn=True):
        ...
        layers = [Rse_block(32, 128, bn=True, single=True),
...
class SUNET(nn.Module):
    def __init__(self, in_ch=3, out_ch=2, ss=True, num_puzzle=9, bench=False):
        ...
        if ss:
            ...
            self.opt_ext = optim.Adam(p0+p1+p2, lr=0.0001)
            # self.opt_ext = optim.Adam(list(self.extractor0.parameters())+list(self.extractor1.parameters())+list(self.extractor2.parameters()), lr=0.00001)
