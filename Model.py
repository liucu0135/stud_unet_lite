...
class SUNET(nn.Module):
    ...
    def forward(self, data):
        ...
        if self.in_ch == 3:
            ...
            # e1 = self.E1(self.input)+e10
        else:
            e1 = self.E1(self.input)
            e1 = torch.cat((e1,e1),dim=1)
        s1 = self.short1(e1)
        ...
        if self.in_ch == 5:  # forcing to else for test here
            e31 = self.E3(e21)
            e32 = self.E3(e22)
            e3 = torch.max(e31, e32)
            # e3 = self.comp3(torch.cat((e31, e32), dim=1))
        else:
            e3 = self.E3(e2)
        s3 = self.short3(e3)
        e4 = self.E4(e3)
    ...
    def save_net(self, save_path):
        torch.save(self.cpu().state_dict(), save_path + '/net_fusion_ave.path')
        self.cuda()
